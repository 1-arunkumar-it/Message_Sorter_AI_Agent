...
Your task is to build a **Priority Inbox agent** for this platform. For every incoming message, the agent must decide whether the user should be interrupted now, whether the message can be batched into a digest, whether it is repetitive enough to mute, or whether it should trigger a safety warning.
